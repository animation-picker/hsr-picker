<script>
	...
	import { t, time } from 'svelte-i18n';
	import {
		activeVersion,
		activePhase,
		viewportHeight,
	...
	import { draw } from '$lib/helpers/pool/pool';
	...
	const doRoll = async (count, bannerToRoll) => {
		...
		console.log('bannerToRoll', bannerToRoll);
		console.log('indexOfBanner', indexOfBanner);
		
		if (bannerToRoll === 'character-event' && indexOfBanner === 0) {
			// let result = {
			// 		// banner: bannerToRoll,
			// 		// bannerId: "35-1", 
			// 		isNew: false, 
			...
			result.forEach(({ name }, index) => {
				tmp.push({
					...
					name,
					pity: 1,
					// rarity: index === 0 || index === 1 || index === 2 ? 5 : index === 7 || index === 8 || index === 9 ? 4 : 3,
					rarity: [0, 1, 2].includes(index) ? 5 : [7, 8, 9].includes(index) ? 4 : 3,
					type: "lightcone",
					undyingQty: 1,
					undyingType: "embers"
				})
