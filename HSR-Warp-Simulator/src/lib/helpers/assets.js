...
// const my = import.meta.glob('@images/my/14/*.webp', {
// 	// query: { w: '150;450;890', as: 'picture' },
// 	query: { as: 'picture' },
// 	import: 'default',
// 	eager: true	
...
const my = import.meta.glob('@images/my/custom/*.webp', {
	// query: { w: '150;450;890', as: 'picture' },
	query: { as: 'picture' },
	import: 'default',
	eager: true	
...
export const itemList = () => {
	...
	const myImagePath = (key, src) => {
		pathList[`lc/small/${key}`] = src;
		pathList[`lc/medium/${key}`] = src;
		pathList[`lc/large/${key}`] = src;
	};
	...
	Object.keys(imageModules).forEach((key) => {
		...
		} else if (key.match(/my/)) {
			myImagePath(keyNoExt, defaultSrc);
